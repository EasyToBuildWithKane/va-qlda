...
| WeeklyReport/ | `WeeklyReportWorkspace` — tab `weekly` / `overview` embedded. `WeeklyReportTimelineNav` (Sprint / từ ngày–đến ngày + báo cáo đã tạo), `WeeklyReportKpiStrip` (6 thẻ: hoàn thành kỳ, tiến độ, SP, giờ công…), `WeeklyReportExecutiveCard` (tóm tắt + `outcomes` giá trị giao được) |
