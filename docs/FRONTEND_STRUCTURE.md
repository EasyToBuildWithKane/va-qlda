...
| Evaluation forms | `Pages/WorkspaceConfig/EvaluationForms/{Index,Create,Edit}.vue` + `Scoring/{Index,Show}.vue` + `modules/evaluation-form/` — list + KPI + Xuất; wizard 3 tab phẳng (section + `border-b`, không card border L/T/R): thông tin chung nhóm theo khối, tiêu chí + thanh trọng số/chia đều, nhân sự HRM + lọc + chống trùng + «Tải tất cả»; chấm điểm; `/workspace-config/evaluation-forms` |
