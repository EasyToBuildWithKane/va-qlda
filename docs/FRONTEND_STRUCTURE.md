...
| Congnghe (landing `/congnghe`) | `Pages/Congnghe/Index.vue` + `partials/*` — **không** `AppLayout`; `CongnghePageShell.vue` cho form/chi tiết người gửi; header `CongngheNavbar.vue` + menu `CongngheUserMenu.vue` (đề xuất đã gửi, hồ sơ, đăng xuất); form `Proposal.vue`; **người gửi:** `MyProposals.vue`, `MyProposalShow.vue` (`/congnghe/de-xuat-cua-toi`) — cổng Congnghe, **không** sidebar QLDA; **quản lý (admin/lead):** `Pages/Congnghe/Proposals/Index.vue`, `Show.vue` — `AppLayout`, sidebar nhóm «Quản trị» → «Đề xuất phần mềm» |
