...
| | | `system_prompt` | textarea | `weekly_report.llm.system_prompt` — prompt tùy chỉnh; trống = prompt mặc định (tổng hợp kết quả, không liệt kê task). Hệ thống luôn nối hợp đồng JSON đầu ra và gỡ markdown/kí hiệu AI. |
...
| **Chào mừng nhân viên** | `onboarding` | `welcome_enabled` | bool | `va.onboarding_welcome_enabled` — màn hình chào mừng lần đầu; UI: `OnboardingTab` (một viewport, form ngang) + `WelcomePanel` (`max-w-3xl`, hero + thẻ chữ, không badge); reset: `POST /settings/onboarding/reset` · `reset-self` |
