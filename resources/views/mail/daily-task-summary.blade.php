...
<head>
    ...
    <title>Tổng hợp công việc</title>
</head>
<body style="font-family: system-ui, -apple-system, Segoe UI, sans-serif; line-height: 1.5; color: #1e293b; max-width: 640px; margin: 0 auto; padding: 24px;">
    <p style="font-size: 18px; font-weight: 600; margin: 0 0 16px; color: #9A0036;">Tổng hợp công việc hôm nay</p>
    <p>Xin chào <strong>{{ $assignee->full_name ?? $assignee->name }}</strong>,</p>
    <p>Danh sách công việc trong ngày <strong>{{ now()->format('d/m/Y') }}</strong> — dự án <strong>{{ $project->name }}</strong>:</p>
    {!! $tasksTable !!}
