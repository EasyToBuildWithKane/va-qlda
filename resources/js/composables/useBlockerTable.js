...
import { exportBlockers } from '@/composables/useBlockerExport';

/** Chấm mức độ (datagrid tinh gọn — không dùng badge). */
export const BLOCKER_SEVERITY_DOT = {
...
export const BLOCKER_SEVERITY_TEXT = {
...
export const BLOCKER_STATUS_TEXT = {
...
export const BLOCKER_SEVERITY_STYLE = {
...
export const BLOCKER_STATUS_STYLE = {
...
export function useBlockerTable(blockersSource) {
    const search = ref('');
    const filterStatus = ref('all');
    const filterSeverity = ref('all');
    const filterOwner = ref('all');
    const filterRecheckPending = ref(false);
    ...
    const filtered = computed(() => {
        ...
        return rows.value.filter((r) => {
            if (filterRecheckPending.value) {
                if (!(r.needs_recheck || r.recheck_pending || r.can?.recheck)) return false;
            } else if (filterStatus.value === 'active') {
                if (!ACTIVE.has(r.status?.value)) return false;
            } else if (filterStatus.value !== 'all' && r.status?.value !== filterStatus.value) {
                return false;
            }
    ...
    const exportBlockersList = (list, { projectCode = 'DA', projectName = '', format = 'xlsx' } = {}) => {
        exportBlockers({ list, projectCode, projectName, format });
    };
    ...
    return {
        ...
        resetFilters,
        exportBlockersList,
        exportRisk: exportBlockersList,
        TERMINAL,
        ACTIVE,
    };
}

/** @deprecated Bí danh cho một phiên bản — dùng useBlockerTable */
export const useRiskTable = useBlockerTable;
export const RISK_SEVERITY_DOT = BLOCKER_SEVERITY_DOT;
export const RISK_SEVERITY_TEXT = BLOCKER_SEVERITY_TEXT;
export const RISK_STATUS_TEXT = BLOCKER_STATUS_TEXT;
export const RISK_SEVERITY_STYLE = BLOCKER_SEVERITY_STYLE;
export const RISK_STATUS_STYLE = BLOCKER_STATUS_STYLE;
