...
export const IMPORT_TEMPLATE_MARKER = 'VA_BLOCKER_IMPORT_V1';
/** Chấp nhận marker cũ để tương thích ngược với file đã xuất trước đó. */
const IMPORT_TEMPLATE_MARKER_LEGACY = 'VA_IMPORT_V1';
 ...
 */
export function downloadBlockerImportTemplate({
    projectCode = 'DA',
    projectName = '',
    severityOptions = [],
    statusOptions = [],
    ...
    XLSX.writeFile(wb, `VA_MauNhap_VuongMac_${code}.xlsx`);
}

/** @deprecated Bí danh — dùng downloadBlockerImportTemplate */
export const downloadRiskImportTemplate = downloadBlockerImportTemplate;
...
function findHeaderIndex(rows) {
    for (let i = 0; i < Math.min(rows.length, 40); i++) {
        ...
        if (row.some((c) => {
            ...
            return s === IMPORT_TEMPLATE_MARKER || s === IMPORT_TEMPLATE_MARKER_LEGACY;
        })) {
            return i + 1;
        }
    }
 ...
 */
export async function parseBlockerImportFile(file) {
...
export function exportPreviewRows(rows, { projectCode = 'DA', projectName = '', mode = 'errors' } = {}) {
    ...
    const code = safeCode(projectCode);
    const t = fileStamp();
    XLSX.writeFile(wb, `VA_VuongMac_${mode === 'errors' ? 'Loi' : 'HopLe'}_${code}_${t.dd}${t.mm}${t.yyyy}.xlsx`);
}

/** @deprecated Bí danh — dùng parseBlockerImportFile */
export const parseRiskImportFile = parseBlockerImportFile;
