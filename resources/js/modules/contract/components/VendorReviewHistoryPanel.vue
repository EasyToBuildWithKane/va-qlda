...
import VendorReviewSummaryBar from '@/modules/contract/components/VendorReviewSummaryBar.vue';
...
const reviewSummary = computed(() => {
    ...
    return {
        ...
        high_count: scores.filter((s) => s >= 8.5).length,
        low_count: scores.filter((s) => s < 7).length,
    };
});
...
function onQuickFilter({ scoreBand }) {
    filters.scoreBand = scoreBand ?? '';
}
...
<template>
  <div class="space-y-5">
    <VendorReviewSummaryBar
      :summary="reviewSummary"
      :active-score-band="filters.scoreBand"
