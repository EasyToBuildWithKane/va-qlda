...
const canDelete = computed(() => props.session.can?.delete === true);
...
<template>
  <div class="relative flex justify-end">
    <button
      ...
      :aria-expanded="open"
      aria-haspopup="menu"
      :title="`Thao tác buổi ${session.session_number}`"
      :aria-label="`Thao tác buổi ${session.session_number}`"
      @click.stop="toggle"
    >
      <AppIcon
    ...
    <Teleport to="body">
      <div
        ...
        role="menu"
        :aria-label="`Menu thao tác buổi ${session.session_number}`"
      >
        <button
          type="button"
          role="menuitem"
