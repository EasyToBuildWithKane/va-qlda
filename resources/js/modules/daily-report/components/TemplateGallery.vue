      ...
      >
        <div class="card w-full max-w-2xl shadow-elevation-3">
          ...
          <div class="flex items-start justify-between border-b border-slate-100 p-5">
            ...
            <button
              class="btn-ghost px-2 py-1 text-slate-400"
              aria-label="Đóng"
              @click="close"
            >
              ✕
          ...
          <div class="flex gap-1 border-b border-slate-100 px-5 pt-3">
            ...
            >
              ...
              <span
                v-if="saved.length"
                class="ml-1 rounded-full bg-slate-200 px-1.5 text-xs text-slate-600"
              >{{ saved.length }}</span>
            </button>
          </div>
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      v-if="tpl.badge"
                      class="rounded-full bg-accent/15 px-2 py-0.5 text-[10px] font-semibold text-brand"
                    >{{ tpl.badge }}</span>
                  </div>
                  <p class="mt-0.5 text-sm text-slate-500">
                    {{ tpl.desc }}
            ...
            >
              <div class="rounded-card border border-dashed border-slate-300 bg-slate-50 p-4">
                ...
                <div class="flex gap-2">
                  ...
                  <button
                    class="btn-primary shrink-0"
                    :disabled="!newName.trim() || !hasContent"
                    @click="saveCurrent"
                  >
                    Lưu mẫu
                  </button>
                ...
                <p class="mt-1.5 text-xs text-slate-400">
                  Mẫu được lưu trên trình duyệt của thiết bị này.
                </p>
              </div>
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="mt-0.5 text-xs text-slate-400">
                    Đã lưu ngày {{ fmtDate(tpl.savedAt) }}
                  </p>
                </div>
                <button
                  class="btn-ghost shrink-0 text-danger hover:bg-danger/10"
                  @click="remove(tpl)"
                >
                  Xóa
                </button>
