...
<template>
  <section>
    ...
    <div
      v-if="editing"
      class="overflow-hidden rounded-xl border border-brand/30 bg-white dark:bg-slate-900"
    >
      ...
      <textarea
        v-model="body"
        rows="8"
        class="w-full resize-y border-0 bg-transparent px-3 py-2 text-sm focus:ring-0"
        placeholder="Markdown, checklist, liên kết…"
      />
      <div class="flex justify-end gap-2 border-t border-slate-100 px-3 py-2 dark:border-slate-800">
        <button
          type="button"
          class="btn-ghost text-xs"
          @click="editing = false; body = props.modelValue || ''"
        >
          Huỷ
        </button>
        <button
