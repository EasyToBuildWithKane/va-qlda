...
<template>
  <div
    v-if="!file"
    class="flex h-full min-h-[200px] items-center justify-center text-slate-400"
  >
    <p class="text-sm">
  ...
  >
    ...
    <div
      v-else-if="error"
      class="flex h-full min-h-[200px] flex-col items-center justify-center px-6 text-center"
    >
      <AppIcon
        name="info"
        :size="32"
    ...
    >
      <div
        ...
        class="docx-wrapper text-sm text-slate-800"
      />
    </div>

    <div
      v-else-if="kind === 'xlsx'"
      class="flex h-full min-h-[320px] flex-col overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-slate-600"
    >
      <div
