...
import {
    downloadBlockerImportTemplate,
    parseBlockerImportFile,
    validateImportRows,
    createPreviewRows,
...
import { exportRiskBlockers } from '@/composables/useBlockerExport';
import { reconcileBlockers } from '@/composables/useBlockerReconcile';
...
const props = defineProps({
    ...
    /** Danh sách vướng mắc hiện tại (dùng cho tab Xuất và Đối soát) */
...
const importDraft = useModalFormDraft('blocker-import', {
...
const saveDraftOnClose = () => { importDraft.saveOnClose({}); };
...
const onFileChange = async (e) => {
    ...
    try {
        ...
        const { rows: raw, errors } = await parseBlockerImportFile(file);
...
const onRowChange = (row) => {
    ...
    if (row.valid && previewTab.value === 'invalid') previewTab.value = 'valid';
};
...
const chooseOtherFile = () => {
    if (!previewRows.value.length) { resetPreview(); return; }
    confirmDelete(
...
const exportRows = (mode) => {
    ...
    exportPreviewRows(list, { projectCode: props.projectCode, projectName: props.projectName, mode });
};
...
const exportRowCount = computed(() => props.blockers.length);

const runExport = (format) => {
    const list = props.blockers;
    if (!list.length) return;
    exportRiskBlockers({ list, projectCode: props.projectCode, projectName: props.projectName, format });
};

// ────────── Reconcile tab ──────────
const reconcileResult = computed(() => reconcileBlockers(props.blockers));
const reconcileIssues = computed(() => reconcileResult.value.issues);
const reconcileSummary = computed(() => reconcileResult.value.summary);
const reconcileFilter = ref('all');
const filteredIssues = computed(() => {
    if (reconcileFilter.value === 'all') return reconcileIssues.value;
    return reconcileIssues.value.filter((i) => i.level === reconcileFilter.value);
});

const levelLabel = { error: 'Lỗi', warning: 'Cảnh báo', info: 'Ghi chú' };
const levelTone = { error: 'text-rose-600 bg-rose-50 dark:bg-rose-950/30', warning: 'text-amber-700 bg-amber-50 dark:bg-amber-950/30', info: 'text-sky-600 bg-sky-50 dark:bg-sky-950/20' };
const levelIcon = { error: 'blockers', warning: 'alert', info: 'info' };
</script>

<template>
  <Modal
    :show="show"
    :dirty="importDirty && activeTab === 'import'"
    title="Dữ liệu vướng mắc"
    max-width="max-w-5xl"
    :on-save-draft="activeTab === 'import' ? saveDraftOnClose : undefined"
    @close="close"
  >
    <!-- ── Tab bar ── -->
    <div class="mb-5 flex gap-1 rounded-xl border border-slate-200 bg-slate-50 p-1 dark:border-slate-700 dark:bg-slate-800/50">
      <button
        v-for="t in [
        ...
        :class="activeTab === t.key
          ? 'bg-white text-brand shadow-sm dark:bg-slate-900 dark:text-brand-100'
    ...
    <div v-if="activeTab === 'import'">
      <template v-if="!canManage">
        <div class="rounded-xl border border-slate-200 bg-slate-50 px-5 py-8 text-center dark:border-slate-700 dark:bg-slate-800/40">
          <AppIcon
            name="blockers"
            :size="28"
          ...
          <p class="mt-1 text-sm text-slate-500">
            Chỉ quản trị viên dự án mới có thể nhập dữ liệu.
          </p>
        </div>
      </template>
      <template v-else>
        ...
        <div class="mt-3 flex flex-col gap-2">
          <label class="flex cursor-pointer items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
            <input
              ...
              type="radio"
              value="filtered"
              class="text-brand focus:ring-brand/30"
            >
            Tất cả vướng mắc trong dự án
            <span class="ml-auto text-xs font-medium text-slate-400">{{ exportRowCount }} mục</span>
          </label>
        </div>
      </div>
    ...
    >
      ...
      <div class="flex flex-wrap gap-2">
        <button
          ...
          :class="reconcileFilter === 'all' ? 'bg-slate-700 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300'"
          @click="reconcileFilter = 'all'"
        >
          Tất cả ({{ reconcileSummary.total }})
        </button>
        <button
          v-if="reconcileSummary.errors"
          type="button"
          class="rounded-full px-3 py-1 text-xs font-semibold transition"
          :class="reconcileFilter === 'error' ? 'bg-rose-600 text-white' : 'bg-rose-50 text-rose-700 hover:bg-rose-100 dark:bg-rose-950/30 dark:text-rose-400'"
          @click="reconcileFilter = 'error'"
        >
          Lỗi ({{ reconcileSummary.errors }})
        </button>
        <button
          v-if="reconcileSummary.warnings"
          type="button"
          class="rounded-full px-3 py-1 text-xs font-semibold transition"
          :class="reconcileFilter === 'warning' ? 'bg-amber-600 text-white' : 'bg-amber-50 text-amber-700 hover:bg-amber-100 dark:bg-amber-950/30 dark:text-amber-400'"
          @click="reconcileFilter = 'warning'"
        >
          Cảnh báo ({{ reconcileSummary.warnings }})
        </button>
        <button
          v-if="reconcileSummary.info"
          type="button"
          class="rounded-full px-3 py-1 text-xs font-semibold transition"
          :class="reconcileFilter === 'info' ? 'bg-sky-600 text-white' : 'bg-sky-50 text-sky-700 hover:bg-sky-100 dark:bg-sky-950/20 dark:text-sky-400'"
          @click="reconcileFilter = 'info'"
        >
          Ghi chú ({{ reconcileSummary.info }})
        </button>
      </div>
      ...
      <div
        ...
        class="rounded-xl border border-dashed border-emerald-200 bg-emerald-50/50 py-10 text-center dark:border-emerald-800/40 dark:bg-emerald-950/10"
      >
        <AppIcon
          name="done"
          :size="28"
        ...
        <p class="mt-1 text-sm text-slate-500">
          Dữ liệu vướng mắc dự án đang sạch.
        </p>
      </div>

      <div
        v-else
        class="max-h-[min(24rem,50vh)] space-y-2 overflow-auto pr-1"
      >
        <div
          ...
          :class="{
            'border-rose-200 bg-rose-50/60 dark:border-rose-800/40 dark:bg-rose-950/10': issue.level === 'error',
            'border-amber-200 bg-amber-50/60 dark:border-amber-800/40 dark:bg-amber-950/10': issue.level === 'warning',
            'border-sky-200 bg-sky-50/60 dark:border-sky-800/40 dark:bg-sky-950/10': issue.level === 'info',
          }"
        >
          <span
            class="mt-0.5 shrink-0 rounded-md p-1 text-xs font-bold"
            :class="levelTone[issue.level]"
          >
            <AppIcon
              :name="levelIcon[issue.level]"
              :size="13"
            />
          </span>
