...
<template>
  <div>
    ...
    <div
      v-if="tasks.length === 0"
      class="grid place-items-center rounded-card border border-dashed border-slate-200 py-20 text-sm text-slate-400"
    >
      Chưa có công việc nào.
    </div>
    <div
      v-else-if="scheduled().length === 0"
      class="grid place-items-center rounded-card border border-dashed border-slate-200 py-20 text-sm text-slate-400"
    >
      <p>Chưa có công việc nào được lên lịch.</p>
      <p class="mt-1 text-xs text-slate-400">
        Hãy đặt ngày bắt đầu &amp; kết thúc cho công việc để hiển thị trên biểu đồ.
      </p>
    </div>
