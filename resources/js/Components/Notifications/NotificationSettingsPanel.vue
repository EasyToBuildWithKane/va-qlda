...
<template>
    <div class="space-y-4">
        ...
        <div v-if="loading" class="py-8 text-center text-sm text-slate-400">Đang tải…</div>

        <template v-else>
            <div class="space-y-2 rounded-xl border border-slate-100 p-3 dark:border-slate-800">
                <h3 class="text-xs font-semibold uppercase tracking-wide text-slate-500">Kênh</h3>
                <label class="flex items-center gap-2 text-[13px]">
                    <input v-model="prefs.channel_in_app" type="checkbox" class="rounded text-brand" />
                    Trong ứng dụng
                </label>
                <label class="flex items-center gap-2 text-[13px] text-slate-400">
                    <input v-model="prefs.channel_email" type="checkbox" disabled class="rounded" />
                    Email <span class="text-[10px]">(sắp ra mắt)</span>
                </label>
                <label class="flex items-center gap-2 text-[13px] text-slate-400">
                    <input v-model="prefs.channel_push" type="checkbox" disabled class="rounded" />
            ...
            <div class="max-h-[50vh] overflow-y-auto space-y-3">
                ...
                >
                    <h3 class="mb-2 text-xs font-semibold uppercase text-slate-500">{{ category }}</h3>
                    <label
                        v-for="t in list"
                        :key="t.value"
            ...
            <div class="flex gap-2 pt-2">
                <button type="button" class="btn-secondary flex-1" @click="emit('close')">Huỷ</button>
