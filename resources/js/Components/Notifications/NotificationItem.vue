...
const metaLine = computed(() => {
    const parts = [];
    if (props.notification.actor_name) parts.push(props.notification.actor_name);
...
<template>
    ...
    >
        <div v-if="selectable" class="flex items-center shrink-0" @click.stop>
            <input
                type="checkbox"
        ...
        >
            <AppIcon :name="icon" :size="14" :class="notification.is_read ? 'text-slate-400' : 'text-brand'" />
            <span
        ...
        <div class="min-w-0 flex-1">
            <div class="flex items-start gap-1.5">
                <p class="flex-1 text-[12px] font-medium leading-snug text-slate-800 dark:text-slate-100 line-clamp-2">
                    {{ notification.title }}
                </p>
                <span
            ...
            <p v-if="notification.body" class="mt-0.5 text-[11px] leading-snug text-slate-500 line-clamp-1 dark:text-slate-400">
                {{ notification.body }}
            </p>
