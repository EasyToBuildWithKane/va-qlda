        ...
        >
            <div v-if="show" class="fixed inset-0 z-50 bg-slate-900/40" @click.self="emit('close')" @keydown.esc="emit('close')">
                ...
                >
                    <aside
                        v-if="show"
                        class="absolute right-0 top-0 flex h-full w-full flex-col bg-white shadow-elevation-3"
                        :class="width"
                        role="dialog"
                        aria-modal="true"
                    >
                        <header class="flex items-center justify-between gap-4 border-b border-slate-100 px-5 py-4">
                            <h2 class="font-display text-base font-semibold text-slate-800">{{ title }}</h2>
                            <button
                                type="button"
