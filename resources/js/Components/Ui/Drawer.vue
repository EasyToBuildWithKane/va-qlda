...
<template>
    <Teleport to="body">
        ...
        >
            <div v-if="show" class="fixed inset-0 z-50 bg-slate-900/40" @click.self="emit('close')" @keydown.esc="emit('close')">
                <Transition
                    enter-active-class="transition duration-200 ease-out"
                ...
                >
                    <aside
                        ...
                        aria-modal="true"
                    >
                        <header v-if="title" class="flex items-center justify-between gap-4 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
                            <h2 class="font-display text-base font-semibold text-slate-800 dark:text-slate-100">{{ title }}</h2>
                            <button
                                type="button"
