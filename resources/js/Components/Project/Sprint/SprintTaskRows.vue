        ...
        <table class="w-full min-w-[960px] border-separate border-spacing-0 text-sm">
            <thead>
                <tr class="text-left text-[10px] font-semibold uppercase tracking-wider text-slate-400">
                    ...
                    <th class="w-11 border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">ID</th>
                    <th class="min-w-[9rem] border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">Công việc</th>
                    <th class="min-w-[6.5rem] border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">Người làm</th>
                    <th class="w-[6.5rem] border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">Trạng thái</th>
                    <th v-if="!compact" class="w-20 border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">Ưu tiên</th>
                    <th class="w-[4.5rem] border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">Bắt đầu</th>
                    <th class="w-[4.5rem] border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">Hạn</th>
                    <th class="w-14 border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">Giờ ƯT</th>
                    <th class="min-w-[7.5rem] border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">SLA</th>
                    <th class="w-[4.5rem] border-b border-slate-100 px-2 py-1.5 dark:border-slate-800">Tiến độ</th>
                    <th class="w-7 border-b border-slate-100 dark:border-slate-800" />
                </tr>
            </thead>
            <tbody v-if="!visibleRows.length">
                <tr>
                    <td :colspan="colCount + 1" class="py-3 text-center text-xs text-slate-400">Chưa có task.</td>
                </tr>
            </tbody>
            <tbody v-else>
                <tr
                ...
                >
                    <td class="border-b border-slate-100 px-1 py-2 dark:border-slate-800" @click.stop>
                        <button
                        ...
                        >
                            <AppIcon :name="entry.childrenOpen ? 'chevron-down' : 'chevron-right'" :size="14" />
                        </button>
                    </td>
                    <td class="border-b border-slate-100 px-2 py-2 font-mono text-xs dark:border-slate-800" :class="entry.isSubtask ? 'text-slate-400' : 'text-slate-500'">
                        <span v-if="entry.isSubtask" class="text-slate-300">↳</span>
                        #{{ entry.task.id }}
                    </td>
                    <td class="border-b border-slate-100 py-2 pr-2 dark:border-slate-800" :class="entry.isSubtask ? 'pl-6' : 'pl-2'">
                    ...
                    <td class="border-b border-slate-100 px-2 py-2 dark:border-slate-800">
                        <template v-if="!entry.isSubtask">
                            <div class="flex min-w-0 items-center gap-1.5">
                                ...
                                <span class="truncate text-xs text-slate-600 dark:text-slate-300" :title="assigneeNames(entry.task)">
                    ...
                    <td class="border-b border-slate-100 px-2 py-2 dark:border-slate-800" @click.stop>
                        ...
                        <span v-else class="flex items-center gap-1 text-xs">
                            <span class="h-2 w-2 shrink-0 rounded-full" :class="statusDot[entry.task.status?.color] || 'bg-slate-400'" />
                            {{ entry.task.status?.label }}
                        </span>
                    </td>
                    <td v-if="!compact" class="border-b border-slate-100 px-2 py-2 dark:border-slate-800">
                        <Badge v-if="!entry.isSubtask" :label="entry.task.priority?.label" :color="entry.task.priority?.color" />
                        <span v-else class="text-[10px] text-slate-400">—</span>
                    </td>
                    <td class="border-b border-slate-100 px-2 py-2 text-xs dark:border-slate-800" :class="entry.isSubtask ? 'text-slate-400 italic' : 'text-slate-500'">
                        <template v-if="entry.isSubtask">Theo cha</template>
                        <template v-else>{{ date(scheduleFor(entry.task).start_date) || '—' }}</template>
                    </td>
                    <td
                    ...
                    >
                        <template v-if="entry.isSubtask">{{ date(scheduleFor(entry.task, entry.parent).due_date) || '—' }}</template>
                        <template v-else>{{ date(scheduleFor(entry.task).due_date) || '—' }}</template>
                    </td>
                    <td class="border-b border-slate-100 px-2 py-2 text-xs font-medium dark:border-slate-800" :class="entry.isSubtask ? 'text-brand' : 'text-slate-500'">
                    ...
                    <td class="border-b border-slate-100 px-2 py-2 dark:border-slate-800">
                        ...
                        <span v-else class="text-[10px] text-slate-400">Theo cha</span>
                    </td>
                    <td class="border-b border-slate-100 px-2 py-2 dark:border-slate-800">
                        <div v-if="!entry.isSubtask" class="flex items-center gap-1">
                            <ProgressBar :value="entry.task.progress" :show-label="false" class="min-w-[2.5rem] flex-1" />
                            <span class="w-8 shrink-0 text-right text-[10px] font-medium text-slate-500">{{ entry.task.progress }}%</span>
                        </div>
                        <span v-else class="text-[10px] text-slate-400">{{ entry.task.progress }}%</span>
                    </td>
                    <td class="border-b border-slate-100 px-2 py-2 dark:border-slate-800" @click.stop>
