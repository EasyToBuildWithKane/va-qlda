...
import TaskDetailAttachments from '@/Components/Project/TaskDetail/TaskDetailAttachments.vue';
import TaskDetailCollaboration from '@/Components/Project/TaskDetail/TaskDetailCollaboration.vue';
import TaskDetailRichEditor from '@/Components/Project/TaskDetail/TaskDetailRichEditor.vue';
import TaskDetailSubtasks from '@/Components/Project/TaskDetail/TaskDetailSubtasks.vue';
...
<template>
    <Teleport to="body">
        <Transition name="fade">
            <div v-if="activeTask" class="fixed inset-0 z-[180] flex justify-end">
                ...
                >
                    ...
                    <header class="shrink-0 border-b border-slate-100 px-4 pb-3 pt-3 dark:border-slate-800">
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-1.5">
                                    ...
                                    <Badge v-if="activeTask.status?.label" :label="activeTask.status.label" :color="activeTask.status.color" />
                                    <Badge v-if="activeTask.priority?.label" :label="activeTask.priority.label" :color="activeTask.priority.color" />
                                    <Badge v-if="activeTask.phase?.label" :label="activeTask.phase.label" color="violet" />
                                </div>
                                <h1 class="mt-1.5 font-display text-lg font-bold leading-snug text-slate-900 dark:text-white">
                                    {{ activeTask.title }}
                                </h1>
                                <p v-if="ws.headerContext" class="mt-1 text-xs text-slate-500">{{ ws.headerContext }}</p>
                                <p v-if="scheduleLine" class="mt-0.5 text-xs text-slate-400">{{ scheduleLine }}</p>
                                <p v-if="activeTask.parent?.id" class="mt-1 text-xs text-slate-500">
                                    Thuộc
                                    <button type="button" class="font-medium text-brand hover:underline" @click="openSubtask(activeTask.parent)">
                            ...
                            <div class="flex shrink-0 items-center gap-0.5">
                                ...
                                >
                                    <AppIcon :name="fullscreen ? 'chevron-right' : 'template'" :size="15" />
                                </button>
                                <button
                                    type="button"
                                    class="grid h-7 w-7 place-items-center rounded-md text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
                        ...
                        <div class="mt-2.5 flex flex-wrap items-center gap-1">
                            ...
                            <div v-if="canEdit" class="relative">
                                ...
                                >
                                    <button type="button" class="w-full px-3 py-1.5 text-left text-xs text-slate-500 hover:bg-slate-50" @click="onAssignPick(null)">— Chưa gán —</button>
                                    <button
                            ...
                            <button type="button" class="grid h-7 w-7 place-items-center rounded-md border border-slate-200 text-slate-500 hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800" title="Bình luận" @click="goComment">
                                <AppIcon name="comment" :size="14" />
                            </button>
                            <button type="button" class="grid h-7 w-7 place-items-center rounded-md border border-slate-200 text-slate-500 hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800" title="Sao chép link" @click="copyLink">
                    ...
                    <!-- Blockers alert -->
                    <div v-if="blockerList.length" class="mx-4 mt-3 shrink-0 space-y-2">
                        <div
                        ...
                        >
                            <div class="flex items-start gap-2">
                                <AppIcon name="blockers" :size="16" class="mt-0.5 shrink-0 text-amber-600" />
                                <div class="min-w-0">
                                    <p class="text-xs font-semibold text-amber-900 dark:text-amber-100">{{ b.code }} · {{ b.title }}</p>
                                    <p v-if="b.root_cause" class="mt-0.5 text-[11px] text-amber-800/90">{{ b.root_cause }}</p>
                    ...
                    <nav class="mt-3 flex shrink-0 gap-0.5 overflow-x-auto border-b border-slate-100 px-4 dark:border-slate-800">
                        <button
                            ...
                            @click="tab = pt.key"
                        >
                            <AppIcon :name="pt.icon" :size="14" />
                            {{ pt.label }}
                            <span v-if="pt.key === 'collaboration' && commentList.length" class="rounded-full bg-slate-200 px-1.5 text-[10px] dark:bg-slate-700">
                                {{ commentList.length }}
                            </span>
                        </button>
                    </nav>
                    ...
                    <div class="min-h-0 flex-1 overflow-y-auto px-4 py-4">
                        ...
                        <div v-show="tab === 'overview'" class="space-y-5">
                            ...
                            <section class="rounded-lg border border-slate-200/80 p-3 dark:border-slate-700">
                                ...
                                <ProgressBar :value="progressPct" class="h-2" />
                                <p v-if="timeSummary" class="mt-2 text-xs text-slate-500">{{ timeSummary }}</p>
                                <ul v-if="worklogList.length" class="mt-2 space-y-1 border-t border-slate-100 pt-2 dark:border-slate-800">
                                    <li v-for="w in worklogList.slice(0, 3)" :key="w.id" class="flex justify-between text-[11px] text-slate-500">
                                    ...
                                    <li v-if="worklogList.length > 3" class="text-[10px] text-slate-400">
                            ...
                            <section class="rounded-lg border border-slate-200/80 bg-slate-50/50 p-3 dark:border-slate-700 dark:bg-slate-900/50">
                                <h3 class="mb-2 text-xs font-semibold text-slate-500">Phân công</h3>
                                <div class="space-y-3">
                                    <div v-if="activeTask.reporter" class="flex items-center gap-3">
                                        <Avatar :name="activeTask.reporter.name" :src="activeTask.reporter.avatar_path" :size="36" />
                                        <div>
                                            <p class="text-sm font-medium text-slate-800 dark:text-slate-100">{{ activeTask.reporter.name }}</p>
                                            <p class="text-[10px] uppercase text-slate-400">Người giao</p>
                                        </div>
                                    </div>
                                    <div v-for="a in assigneeList" :key="a.id" class="flex items-center gap-3">
                                        <Avatar :name="a.name" :src="a.avatar_path" :size="36" />
                                        <div>
                                            <p class="text-sm font-medium text-slate-800 dark:text-slate-100">{{ a.name }}</p>
                                            <p class="text-[10px] uppercase text-slate-400">Người thực hiện</p>
                                        </div>
                                    </div>
                                    <p v-if="!assigneeList.length && !activeTask.reporter" class="text-sm text-slate-400">Chưa phân công.</p>
                                    <div v-if="activeTask.reviewer" class="flex items-center gap-3 border-t border-slate-200/80 pt-3 dark:border-slate-700">
                                        <Avatar :name="activeTask.reviewer.name" :src="activeTask.reviewer.avatar_path" :size="36" />
                                        <div>
                                            <p class="text-sm font-medium text-slate-800 dark:text-slate-100">{{ activeTask.reviewer.name }}</p>
                                            <p class="text-[10px] uppercase text-slate-400">Người duyệt</p>
                                        </div>
                                    </div>
                                    <div v-if="watcherList.length" class="border-t border-slate-200/80 pt-3 dark:border-slate-700">
                                        <p class="mb-2 text-[10px] uppercase text-slate-400">Người theo dõi</p>
                                        <div class="flex flex-wrap gap-1">
                                            ...
                                            >
                                                <Avatar :name="w.name" :src="w.avatar_path" :size="18" />
                                                {{ w.name }}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            >
                                ...
                                />
                                <p v-else class="rounded-xl border border-dashed border-slate-200 py-8 text-center text-sm text-slate-400">Chưa có mô tả.</p>
                            </TaskDetailRichEditor>
                            ...
                            <!-- Related -->
                            <section v-if="relatedBlockedByList.length || relatedBlockingList.length" class="rounded-xl border border-slate-200/80 p-4 dark:border-slate-700">
                                <h3 class="mb-3 text-xs font-bold uppercase tracking-wide text-slate-500">Liên kết công việc</h3>
                                <div v-if="relatedBlockedByList.length" class="mb-3">
                                    <p class="mb-1.5 text-[10px] font-semibold uppercase text-slate-400">Phụ thuộc (Blocked by)</p>
                                    <button
                                    ...
                                    >
                                        ...
                                        <Badge v-if="r.status" :label="r.status.label" :color="r.status.color" />
                                    </button>
                                </div>
                                <div v-if="relatedBlockingList.length">
                                    <p class="mb-1.5 text-[10px] font-semibold uppercase text-slate-400">Đang chặn (Blocking)</p>
                                    <button
                        ...
                        <!-- ACTIVITY -->
                        <div v-show="tab === 'activity'" class="space-y-0">
                            <div
                            ...
                            >
                                <div class="relative flex flex-col items-center">
                                    ...
                                    >
                                        <AppIcon :name="ev.icon" :size="14" />
                                    </span>
                                    <span class="mt-1 w-px flex-1 bg-slate-200 dark:bg-slate-700" />
                                </div>
                                <div class="min-w-0 flex-1 pt-0.5">
                                    <p class="text-sm font-semibold text-slate-800 dark:text-slate-100">{{ ev.title }}</p>
                                    <p class="mt-0.5 text-xs text-slate-500">{{ datetime(ev.at) }}</p>
                                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">{{ ev.detail }}</p>
                                </div>
                            </div>
                            <p v-if="!activityList.length" class="py-8 text-center text-sm text-slate-400">Chưa có hoạt động ghi nhận.</p>
                        </div>

                        <!-- LINKS / DOCS -->
                        <div v-show="tab === 'links'" class="space-y-4">
                            <p class="text-sm text-slate-500">Tài liệu dự án — truy cập nhanh từ workspace task.</p>
                            <div v-for="(g, gIdx) in projectDocList" :key="g.key ?? `doc-${gIdx}`" class="rounded-xl border border-slate-200/80 dark:border-slate-700">
                                <div class="border-b border-slate-100 px-3 py-2 dark:border-slate-800">
                                    <p class="text-sm font-semibold text-slate-800 dark:text-slate-100">{{ g.label }}</p>
                                    <p class="text-[10px] text-slate-400">{{ g.hint }}</p>
                                </div>
                                <ul class="divide-y divide-slate-100 dark:divide-slate-800">
                                    <li v-for="f in g.files" :key="f.id">
                                        <a
                                            :href="f.url"
                                            target="_blank"
                                            rel="noopener"
                                        ...
                                        >
                                            <AppIcon name="documents" :size="16" class="shrink-0 text-slate-400" />
                                            <span class="min-w-0 flex-1 truncate">{{ f.original_name }}</span>
                                            <AppIcon name="download" :size="14" class="shrink-0 text-slate-400" />
                                        </a>
                                    </li>
                                </ul>
                            </div>
                            <p v-if="!projectDocList.length" class="rounded-xl border border-dashed border-slate-200 py-10 text-center text-sm text-slate-400">
