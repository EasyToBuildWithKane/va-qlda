...
<template>
    <Modal :show="show" :dirty="dirty" title="Dữ liệu Sprint & Task" max-width="max-w-5xl" @close="close">
        <!-- Tabs -->
        ...
        <!-- ===== NHẬP ===== -->
        <div v-if="tab === 'import'" class="space-y-4">
            <div v-if="!canManage" class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                Bạn không có quyền nhập dữ liệu.
            </div>
            <template v-else>
                <div class="flex items-center gap-2 text-xs">
                    <span class="rounded-full px-2.5 py-1 font-semibold" :class="importStep === 'guide' ? 'bg-brand text-white' : 'bg-slate-100 text-slate-500'">1. Hướng dẫn</span>
                    <AppIcon name="chevron-right" :size="12" class="text-slate-300" />
                    <span class="rounded-full px-2.5 py-1 font-semibold" :class="importStep === 'preview' ? 'bg-brand text-white' : 'bg-slate-100 text-slate-500'">2. Xem trước & nhập</span>
                </div>

                <div v-if="importStep === 'guide'" class="space-y-4">
                    <div class="rounded-xl border border-brand/20 bg-brand/5 p-4">
                        <h3 class="flex items-center gap-2 font-semibold text-slate-800 dark:text-slate-100">
                            <AppIcon name="info" :size="16" class="text-brand" />
                            Quy trình nhập
                        </h3>
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                        <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                            <p class="text-xs font-semibold uppercase text-slate-400">Cột bắt buộc</p>
                            <ul class="mt-2 space-y-1 text-sm text-slate-600">
                                <li v-for="h in SPRINT_IMPORT_HEADERS.filter((c) => c.label.includes('*'))" :key="h.key">{{ h.label }}</li>
                            </ul>
                        </div>
                        <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                            <p class="text-xs font-semibold uppercase text-slate-400">Lưu ý</p>
                            <p class="mt-2 text-sm text-slate-600">Tối đa 200 dòng/lần · Xóa dòng mẫu in nghiêng trước khi upload.</p>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-2">
                        <button type="button" class="btn-primary text-sm" @click="onDownloadTemplate">
                            <AppIcon name="download" :size="15" /> Tải file mẫu (.xlsx)
                        </button>
                        <button type="button" class="btn-ghost border border-slate-200 text-sm dark:border-slate-600" @click="onPickFile">
                            <AppIcon name="upload" :size="15" /> Chọn file đã điền
                        </button>
                    </div>
                    <input ref="fileInputRef" type="file" accept=".xlsx,.xls,.csv" class="hidden" @change="onFileChange" />
                    <p v-if="parsing" class="text-sm text-slate-500"><AppIcon name="refresh" :size="14" class="inline animate-spin" /> Đang đọc…</p>
                    <p v-if="fileError" class="text-sm text-rose-600">{{ fileError }}</p>
                    <ul v-if="parseErrors.length" class="list-disc pl-5 text-sm text-amber-700">
                        <li v-for="(e, i) in parseErrors" :key="i">{{ e }}</li>
                    </ul>
                </div>

                <div v-else class="space-y-3">
                    <button type="button" class="btn-ghost text-xs" @click="resetImport">← Quay lại hướng dẫn</button>
                        ...
                        <table class="w-full min-w-[720px] text-xs">
                            <thead class="sticky top-0 bg-slate-50 text-[10px] uppercase text-slate-500 dark:bg-slate-800">
                                <tr>
                                    ...
                                    <th class="px-2 py-1.5">Tiêu đề</th>
                                    <th class="px-2 py-1.5">Sprint</th>
                                    <th class="px-2 py-1.5">TT</th>
                                    <th class="px-2 py-1.5">Ưu tiên</th>
                                    <th class="px-2 py-1.5">Lỗi</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="row in displayedRows" :key="row.line" class="border-t border-slate-100 dark:border-slate-800">
                                    <td class="px-2 py-1">{{ row.line }}</td>
                                    <td class="px-2 py-1">
                                        <input v-model="row.title" class="input w-full py-0.5 text-xs" @blur="onRowBlur(row)" />
                                    </td>
                                    <td class="px-2 py-1">
                                        <input v-model="row.sprint_name" class="input w-full py-0.5 text-xs" @blur="onRowBlur(row)" />
                                    </td>
                                    <td class="px-2 py-1">
                                        <input v-model="row.status" class="input w-20 py-0.5 text-xs" @blur="onRowBlur(row)" />
                                    </td>
                                    <td class="px-2 py-1">
                                        <input v-model="row.priority" class="input w-20 py-0.5 text-xs" @blur="onRowBlur(row)" />
                                    </td>
                                    <td class="px-2 py-1 text-rose-600">{{ row.errors?.join('; ') }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div class="flex justify-end gap-2 border-t border-slate-100 pt-3 dark:border-slate-800">
                        <button type="button" class="btn-ghost text-sm" @click="previewTab = previewTab === 'valid' ? 'invalid' : 'valid'">
        ...
        <!-- ===== XUẤT ===== -->
        <div v-else-if="tab === 'export'" class="space-y-4">
            <div class="rounded-xl border border-brand/20 bg-brand/5 p-4">
                <h3 class="font-semibold text-slate-800 dark:text-slate-100">Xuất Excel định dạng</h3>
                <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">
                    File gồm sheet Tổng quan, Tasks, Sprints — có tiêu đề, màu thương hiệu và bảng dễ đọc.
                </p>
            </div>
            <div>
                <p class="mb-2 text-xs font-semibold uppercase text-slate-400">Phạm vi task</p>
                <div class="flex flex-wrap gap-2">
                    <label class="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm" :class="exportScope === 'filtered' ? 'border-brand bg-brand/5' : 'border-slate-200'">
                        <input v-model="exportScope" type="radio" value="filtered" class="text-brand" />
                        Kết quả tìm kiếm ({{ filteredTasks.length }})
                    </label>
                    <label class="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm" :class="exportScope === 'all' ? 'border-brand bg-brand/5' : 'border-slate-200'">
                        <input v-model="exportScope" type="radio" value="all" class="text-brand" />
                        Toàn bộ dự án ({{ tasks.length }})
                    </label>
                </div>
            </div>
            <p class="text-sm text-slate-500">Luôn kèm {{ sprints.length }} sprint trong file.</p>
            <button type="button" class="btn-primary text-sm" @click="runExport">
                <AppIcon name="export" :size="15" /> Tải file Excel (.xlsx)
            </button>
        </div>
        ...
        <div v-else class="space-y-4">
            ...
            <p v-if="!issues.length" class="py-8 text-center text-sm text-emerald-600">Dữ liệu nhất quán.</p>
            ...
            <button v-if="issues.length" type="button" class="btn-ghost border border-slate-200 text-sm" @click="exportReconcileOnly">
                <AppIcon name="export" :size="14" /> Xuất báo cáo đối soát
            </button>
        </div>

        <div class="mt-4 flex justify-end border-t border-slate-100 pt-3 dark:border-slate-800">
            <button type="button" class="btn-ghost text-sm" @click="close">Đóng</button>
        </div>
    </Modal>
</template>
