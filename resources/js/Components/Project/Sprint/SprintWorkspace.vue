...
import SprintListView from '@/Components/Project/Sprint/SprintListView.vue';
import SprintCalendarView from '@/Components/Project/Sprint/SprintCalendarView.vue';
import TaskDetailPanel from '@/Components/Project/Sprint/TaskDetailPanel.vue';
import SprintDataModal from '@/Components/Project/Sprint/SprintDataModal.vue';
...
<template>
    <div class="flex h-full min-h-0 flex-col bg-slate-50/50 dark:bg-slate-950">
        ...
        <div class="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-slate-200/80 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
            <div>
                <h2 class="font-display text-lg font-bold text-slate-900 dark:text-slate-50">Sprint Workspace</h2>
                <p class="text-xs text-slate-500">Điều phối sprint, task và tiến độ dự án</p>
            </div>
            <button v-if="canManage" type="button" class="btn-primary text-sm" @click="openSprint()">
                <AppIcon name="add" :size="15" /> Sprint
            </button>
        </div>
        ...
        <div class="sticky top-0 z-20 shrink-0 border-b border-slate-200/80 bg-white/95 px-3 py-2 backdrop-blur dark:border-slate-800 dark:bg-slate-900/95">
            <div class="flex flex-wrap items-center gap-2">
                <div class="relative min-w-[12rem] flex-1">
                    <AppIcon name="search" :size="14" class="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input
                ...
                <button type="button" class="btn-ghost border border-slate-200 text-xs dark:border-slate-600" @click="openDataModal('import')">
                    <AppIcon name="download" :size="14" /> Dữ liệu
                    <span v-if="summary.errors" class="ml-1 rounded-full bg-rose-500 px-1 text-[10px] text-white">{{ summary.errors }}</span>
                </button>
                <button v-if="canContribute" type="button" class="btn-primary text-xs" @click="openTaskModal()">
                    <AppIcon name="add" :size="14" /> Task
                </button>
            </div>
        </div>
