...
<template>
    <Modal :show="show" :dirty="importDirty" title="Nhập rủi ro & vướng mắc từ file" max-width="max-w-5xl" @close="close">
        ...
        <!-- Guide step -->
        <div v-if="step === 'guide'" class="space-y-4">
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Cột bắt buộc</p>
                    <ul class="mt-2 space-y-1 text-sm text-slate-600 dark:text-slate-300">
                        <li v-for="h in IMPORT_HEADERS.filter((c) => c.label.includes('*'))" :key="h.key">
                            {{ h.label }}
                        </li>
                    </ul>
                </div>
                <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Mức độ hợp lệ</p>
            ...
            <p v-if="parsing" class="flex items-center gap-2 text-sm text-slate-500">
                <AppIcon name="refresh" :size="14" class="animate-spin" /> Đang đọc file…
            </p>
            <p v-if="fileError" class="text-sm text-rose-600">{{ fileError }}</p>
            <ul v-if="parseErrors.length" class="list-disc pl-5 text-sm text-amber-700 dark:text-amber-400">
        ...
        <div v-else class="space-y-4">
            ...
            <div v-else class="max-h-[min(24rem,50vh)] overflow-auto rounded-xl border border-slate-200 dark:border-slate-700">
                <table class="w-full min-w-[880px] text-xs">
                    <thead class="sticky top-0 z-10 bg-slate-50 text-left text-[10px] uppercase tracking-wide text-slate-500 dark:bg-slate-800">
                        <tr>
                            ...
                            <th class="min-w-[10rem] px-1 py-2">Tiêu đề *</th>
                            <th class="w-28 px-1 py-2">Mức độ *</th>
                            <th class="w-28 px-1 py-2">Trạng thái</th>
                            <th class="w-32 px-1 py-2">Phụ trách</th>
                            <th class="w-32 px-1 py-2">Hạn</th>
                            <th class="min-w-[8rem] px-1 py-2">Nguyên nhân</th>
                            <th class="min-w-[8rem] px-1 py-2">Hướng xử lý</th>
                            <th v-if="previewTab === 'invalid'" class="min-w-[8rem] px-1 py-2">Lỗi</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        ...
                        >
                            <td class="px-1 py-1.5 text-slate-400">{{ row.line }}</td>
                            ...
                            <td v-if="previewTab === 'invalid'" class="px-1 py-1.5 text-[10px] leading-snug text-rose-600">
                                {{ row.errors.join('; ') }}
                            </td>
                            <td class="px-1 py-1.5">
                                <button
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4 dark:border-slate-800">
                <div class="flex flex-wrap gap-2">
                    <button type="button" class="btn-ghost text-sm" @click="modalClose()">
                        Huỷ — không nhập
                    </button>
                    <button
                    ...
                    >
                        <AppIcon name="export" :size="14" /> Xuất {{ invalidRows.length }} dòng lỗi
                    </button>
                    <button
                    ...
                    >
                        <AppIcon name="download" :size="14" /> Xuất {{ validRows.length }} dòng hợp lệ
                    </button>
                </div>
                <button
                    type="button"
                    ...
                    @click="submitImport"
                >
                    <AppIcon v-if="importing" name="refresh" :size="14" class="animate-spin" />
        ...
        <div v-if="step === 'guide'" class="mt-4 flex justify-end border-t border-slate-100 pt-4 dark:border-slate-800">
            <button type="button" class="btn-ghost text-sm" @click="modalClose()">Đóng</button>
        </div>
    </Modal>
</template>
