...
<template>
    <div class="grid gap-4 lg:grid-cols-2">
        <div class="space-y-4">
            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Mô tả chi tiết</p>
                <textarea
                    v-if="canEdit"
                ...
                />
                <p v-else class="mt-1 whitespace-pre-wrap text-sm text-slate-600 dark:text-slate-300">{{ row.description || '—' }}</p>
            </div>

            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Nguyên nhân</p>
                <textarea
                    v-if="canEdit"
                ...
                />
                <p v-else class="mt-1 whitespace-pre-wrap text-sm text-slate-600 dark:text-slate-300">{{ row.root_cause || '—' }}</p>
            </div>

            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Hướng xử lý</p>
                <textarea
                    v-if="canEdit"
                ...
                />
                <p v-else class="mt-1 whitespace-pre-wrap text-sm text-slate-600 dark:text-slate-300">{{ row.resolution || '—' }}</p>
            </div>

            <div v-if="canEdit && dirty" class="flex justify-end">
                <button
                    type="button"
            ...
            <div>
                <div class="flex items-center justify-between gap-2">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Ảnh & file đính kèm</p>
                    <button
                        v-if="canEdit"
                        type="button"
                ...
                <div v-if="attachments.length" class="mt-2 grid gap-2 sm:grid-cols-2">
                    <div
                    ...
                    >
                        <a
                            ...
                            class="block"
                        >
                            <img :src="file.url" :alt="file.original_name" class="aspect-video w-full object-cover" />
                        </a>
                        <a
                            v-else
                            :href="file.url"
                        ...
                        >
                            <AppIcon name="template" :size="18" class="shrink-0 text-slate-400" />
                ...
                <p v-else class="mt-2 rounded-xl border border-dashed border-slate-200 p-3 text-xs text-slate-400 dark:border-slate-600">
        ...
        <div class="space-y-4">
            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Lịch sử cập nhật</p>
                <ul class="mt-2 max-h-48 space-y-2 overflow-y-auto pr-1">
                    ...
                    >
                        <span class="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-slate-100 text-slate-500 dark:bg-slate-700">
                            <AppIcon :name="activityIcon(item.event)" :size="12" />
                        </span>
                        <div class="min-w-0 flex-1">
                            <p class="font-medium text-slate-700 dark:text-slate-200">{{ item.description }}</p>
                ...
                <p v-if="row.resolved_at" class="mt-2 text-xs text-emerald-600 dark:text-emerald-400">
