...
<template>
    <div class="flex items-center gap-2">
        <div class="flex-1 rounded-full bg-slate-100 overflow-hidden" :class="height">
            <div class="h-full rounded-full transition-all duration-300" :class="fill" :style="{ width: pct + '%' }"></div>
        </div>
        <span v-if="showLabel" class="w-9 text-right text-xs font-medium text-slate-500">{{ pct }}%</span>
    </div>
</template>
