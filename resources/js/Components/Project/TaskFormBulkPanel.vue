...
<template>
    <div class="space-y-4">
        ...
        >
            <AppIcon name="info" :size="18" class="mt-0.5 shrink-0 text-brand" />
            <div class="min-w-0 flex-1 space-y-1">
                <p class="font-semibold text-slate-800 dark:text-slate-100">Tạo hàng loạt nhanh</p>
            ...
            <button type="button" class="shrink-0 text-slate-400 hover:text-slate-600" @click="showTips = false">
        ...
        <section class="rounded-lg border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-900/40">
            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div>
                    ...
                    <select v-model="defaults.sprint_id" class="input py-1.5 text-sm">
                ...
                <div>
                    ...
                    <select v-model="defaults.status" class="input py-1.5 text-sm">
                        <option v-for="o in statusOptions" :key="o.value" :value="o.value">{{ o.label }}</option>
                    </select>
                </div>
                <div>
                    <label class="label text-[11px]">Ưu tiên</label>
                    <select v-model="defaults.priority" class="input py-1.5 text-sm">
                        <option v-for="o in priorityOptions" :key="o.value" :value="o.value">{{ o.label }}</option>
                    </select>
                </div>
                <div>
                    <label class="label text-[11px]">Giai đoạn</label>
                    <select v-model="defaults.phase" class="input py-1.5 text-sm">
            ...
            <div class="mt-3">
                ...
                <div class="flex flex-wrap gap-1.5">
                    ...
                    >
                        ...
                        />
                        <Avatar :name="e.name" :src="e.avatar_path" :size="18" />
                        <span>{{ e.name }}</span>
                    </label>
                </div>
            </div>
            <p v-if="selectedSprintName" class="mt-2 text-[11px] text-brand">
        ...
        <div class="flex items-center gap-2 text-xs">
            ...
            <AppIcon name="chevron" :size="14" class="text-slate-300" />
            <button
                type="button"
                class="flex items-center gap-1.5 rounded-full px-3 py-1 font-semibold transition"
        ...
        <!-- Compose -->
        <div v-show="view === 'compose'" class="space-y-2">
            <div class="flex flex-wrap items-center justify-between gap-2">
                <label class="label mb-0">Danh sách tiêu đề <span class="text-rose-500">*</span></label>
                <div class="flex flex-wrap gap-1.5">
                    <button type="button" class="btn-ghost py-1 text-xs" @click="insertSample">
                        <AppIcon name="template" :size="14" /> Chèn mẫu
                    </button>
                    <button type="button" class="btn-ghost py-1 text-xs text-slate-500" :disabled="!bulkText.trim()" @click="clearAll">
                        Xoá hết
                    </button>
                </div>
            </div>
            ...
            <div class="flex justify-end">
                <button type="button" class="btn-primary text-sm" :disabled="!bulkText.trim()" @click="goPreview">
                    Tiếp tục xem trước
        ...
        <!-- Preview -->
        <div v-show="view === 'preview'" class="space-y-3">
            <div class="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs dark:border-slate-700 dark:bg-slate-900">
                ...
                <span v-if="summary.invalid" class="text-amber-700 dark:text-amber-400">{{ summary.invalid }} lỗi</span>
                <span v-if="summary.duplicates" class="text-slate-500">{{ summary.duplicates }} trùng</span>
                <span v-if="summary.exists" class="text-slate-500">{{ summary.exists }} đã tồn tại</span>
                ...
                <ul class="max-h-[min(280px,36vh)] divide-y divide-slate-100 overflow-y-auto dark:divide-slate-800">
                    ...
                    >
                        ...
                        <div class="w-24 shrink-0 text-right">
                            <span v-if="!rowHasErrors(row.id)" class="text-[10px] font-medium text-emerald-600">OK</span>
                            <template v-else>
                                <span
            ...
            <button type="button" class="btn-ghost w-full border border-dashed border-slate-200 text-xs dark:border-slate-700" @click="addEmptyRow">
                <AppIcon name="plus" :size="14" /> Thêm dòng trống
            </button>

            <div class="flex flex-wrap justify-between gap-2 pt-1">
                <button type="button" class="btn-ghost text-sm" @click="backToCompose">
                    <AppIcon name="back" :size="14" /> Sửa danh sách
                </button>
                <div class="flex gap-2">
                    ...
                    <button
                        ...
                        @click="submit"
                    >
                        <AppIcon v-if="!submitting" name="add" :size="15" />
        ...
        <!-- Compose footer -->
        <div v-if="view === 'compose'" class="flex justify-end gap-2 border-t border-slate-100 pt-3 dark:border-slate-800">
            <button type="button" class="btn-ghost" @click="modalClose()">Huỷ</button>
        </div>
    </div>
</template>
