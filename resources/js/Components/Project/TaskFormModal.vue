...
import TaskFormBulkPanel from '@/Components/Project/TaskFormBulkPanel.vue';
...
<template>
    <Modal :show="show" :dirty="modalDirty" :title="modalTitle" max-width="max-w-6xl" @close="emit('close')">
        <!-- Mode tabs (create only) -->
        <div v-if="!task" class="mb-5 flex rounded-lg border border-slate-200 bg-slate-50 p-1 dark:border-slate-700 dark:bg-slate-900/50">
            <button
                type="button"
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="space-y-3">
                    <div>
                        ...
                        <input v-model="form.title" type="text" class="input" placeholder="Mô tả ngắn gọn công việc cần làm…" />
                        <p v-if="form.errors.title" class="mt-1 text-xs text-danger">{{ form.errors.title }}</p>
                    </div>

                    <div>
                        <label class="label">Mô tả chi tiết</label>
                        <textarea v-model="form.description" rows="3" class="input resize-none" placeholder="Yêu cầu, tiêu chí hoàn thành, tài liệu tham khảo…"></textarea>
                    </div>

                    <div class="rounded-card border border-slate-200 p-3 dark:border-slate-700">
                        <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Nhân sự</p>
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                <label class="label">
                                    Người thực hiện
                                    <span class="ml-1 cursor-help text-slate-400" title="Có thể chọn nhiều người cùng thực hiện công việc này.">ⓘ</span>
                                </label>
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    >
                                        ...
                                        />
                                        <Avatar :name="e.name" :src="e.avatar_path" :size="20" />
                                        <span>{{ e.name }}</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <select v-model="form.reporter_id" class="input">
                            ...
                            <div>
                                ...
                                <select v-model="form.reviewer_id" class="input">
                                    <option :value="null">— Chưa chọn —</option>
                                    <option v-for="e in employees" :key="e.id" :value="e.id">{{ e.name }}</option>
                                </select>
                            </div>
                ...
                <div class="space-y-3">
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                        <div>
                            ...
                            <select v-model="form.status" class="input" @change="syncProgressFromStatus">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input v-model="form.estimate_hours" type="number" step="0.5" min="0" class="input" placeholder="8" />
                        </div>
                        <div>
                            <label class="label">Tiến độ: <strong class="text-brand">{{ form.progress_percent }}%</strong></label>
                            <input v-model.number="form.progress_percent" type="range" min="0" max="100" step="5" class="w-full accent-brand" />
                        </div>
                    </div>
                    ...
                    <div v-if="dependencyOptions.length">
                        ...
                        <div class="max-h-32 space-y-1 overflow-y-auto rounded-input border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-900">
                            <label v-for="t in dependencyOptions" :key="t.id" class="flex items-center gap-2 text-sm text-slate-600">
                                <input v-model="form.dependencies" type="checkbox" :value="t.id" class="rounded accent-brand" />
                                {{ t.title }}
                            </label>
                        </div>
                    </div>
