...
<template>
    <section class="rounded-xl border border-slate-200/80 dark:border-slate-700">
        ...
        <ul v-if="subtaskList.length" class="divide-y divide-slate-100 dark:divide-slate-800">
            <li
            ...
            >
                <button
                    ...
                    @click="toggleDone(st)"
                >
                    <AppIcon v-if="st.status?.value === 'done'" name="check" :size="12" />
                </button>
                ...
                <label v-if="canEdit" class="flex shrink-0 items-center gap-1 text-[11px] text-slate-500">
                    <input
                        type="number"
                ...
                <span v-else-if="st.estimate_hours != null" class="shrink-0 text-xs font-medium text-brand">{{ st.estimate_hours }}h</span>
            </li>
        </ul>
        ...
        >
            <input v-model="title" type="text" class="input min-w-[10rem] flex-1 text-sm" placeholder="Tiêu đề công việc con…" />
            <input
                ...
                title="Ước tính giờ"
            />
            <button type="submit" class="btn-primary text-xs" :disabled="adding || !title.trim()">
                <AppIcon name="add" :size="14" />
            </button>
        </form>
        <p v-else-if="isParentSubtask" class="px-3 py-3 text-center text-xs text-slate-400">
            Công việc này là việc con — không thể thêm cấp con thêm.
        </p>
        <p v-else-if="!subtaskList.length" class="px-3 py-4 text-center text-xs text-slate-400">Chưa có công việc con.</p>
    </section>
</template>
