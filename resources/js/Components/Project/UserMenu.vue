    ...
    <div ref="menuRef" class="relative">
        ...
        >
            ...
            <div class="hidden sm:block text-left leading-tight">
                <p class="max-w-[140px] truncate text-[13px] font-semibold text-slate-800">{{ displayName }}</p>
                <p class="text-[11px] text-slate-400 mt-0.5">{{ roleLabel || 'Người dùng' }}</p>
            </div>
            <AppIcon
                name="chevron-down"
                :size="13"
                ...
                <div class="bg-gradient-to-br from-brand via-brand to-brand/80 px-4 pt-4 pb-3.5">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-[14px] font-semibold text-white leading-tight truncate">{{ displayName }}</p>
                            <p v-if="email" class="text-[11.5px] text-white/60 leading-tight mt-0.5 truncate">{{ email }}</p>
                            <p v-else class="text-[11.5px] text-white/40 leading-tight mt-0.5">Chưa có email</p>
                        </div>
                    </div>
                    <!-- Role + status badges -->
                ...
                <div class="p-1.5 space-y-0.5">
                    ...
                    <div class="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-[13px] text-slate-400 cursor-not-allowed select-none">
                        <div class="h-7 w-7 shrink-0 rounded-lg bg-slate-100 flex items-center justify-center">
                            <AppIcon name="account" :size="14" class="text-slate-400" />
                        </div>
                    ...
                    <div class="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-[13px] text-slate-400 cursor-not-allowed select-none">
                        <div class="h-7 w-7 shrink-0 rounded-lg bg-slate-100 flex items-center justify-center">
                            <AppIcon name="system-config" :size="14" class="text-slate-400" />
                        </div>
                ...
                <div class="border-t border-slate-100 p-1.5">
                    ...
                    >
                        <div class="h-7 w-7 shrink-0 rounded-lg bg-rose-50 flex items-center justify-center">
                            <AppIcon name="logout" :size="14" class="text-rose-500" />
                        </div>
                        <span>Đăng xuất</span>
                    </button>
                </div>
