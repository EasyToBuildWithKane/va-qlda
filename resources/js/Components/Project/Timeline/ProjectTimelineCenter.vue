...
import ProjectTimelineView from '@/Components/Project/Timeline/ProjectTimelineView.vue';
        ...
        <div class="flex shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-4 py-2">
            ...
            <div class="flex min-w-0 flex-wrap items-center gap-1.5">
                <span
                    ...
                    :class="chip.chip"
                >
                    <span v-if="chip.dot" class="h-1.5 w-1.5 shrink-0 rounded-full" :class="chip.dot"></span>
                    <span v-else-if="chip.prefix" class="shrink-0">{{ chip.prefix }}</span>
            ...
            <div class="hidden items-center gap-1 sm:flex">
                <button type="button" class="grid h-7 w-7 place-items-center rounded border border-slate-200 text-slate-500 hover:bg-slate-100" @click="zoomOut" title="Thu nhỏ">
                ...
                <button type="button" class="grid h-7 w-7 place-items-center rounded border border-slate-200 text-slate-500 hover:bg-slate-100" @click="zoomIn" title="Phóng to">
                    <AppIcon name="plus" :size="13" />
                </button>
            </div>

            <button v-if="canManage" type="button" class="btn-primary shrink-0 text-xs" @click="emit('create-task')">
                <AppIcon name="add" :size="13" /> Tạo task
            </button>
        </div>
        ...
        <div class="flex min-h-0 flex-1 flex-col overflow-hidden">
            ...
            >
                <div class="mb-4 grid h-16 w-16 place-items-center rounded-2xl bg-brand-50 text-brand">
                    <AppIcon name="timeline" :size="28" />
                </div>
                <h3 class="font-display text-lg font-semibold text-slate-800">Chưa có dữ liệu biểu đồ</h3>
                <p class="mt-1 max-w-sm text-sm text-slate-500">Thêm task có ngày bắt đầu và hạn để xây dựng Gantt chart.</p>
                <button v-if="canManage" type="button" class="btn-primary mt-4 text-sm" @click="emit('create-task')">
                    <AppIcon name="add" :size="14" /> Tạo task đầu tiên
                </button>
            </div>
