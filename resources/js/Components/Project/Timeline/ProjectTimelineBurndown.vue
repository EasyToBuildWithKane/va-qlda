...
<template>
    <div v-if="chart" class="w-full">
        <svg :viewBox="`0 0 ${width} ${height}`" class="w-full" role="img" aria-label="Biểu đồ burndown">
            <line
                v-for="tick in 4"
                :key="tick"
                :x1="pad.l"
                ...
                stroke-width="1"
            />
            <path :d="chart.idealPath" fill="none" class="stroke-slate-300 dark:stroke-slate-600" stroke-width="1.5" stroke-dasharray="4 3" />
            <path :d="chart.actualPath" fill="none" class="stroke-brand" stroke-width="2" />
            <text
        ...
        <div class="mt-1 flex gap-3 text-[10px] text-slate-500">
            <span class="inline-flex items-center gap-1"><span class="h-0.5 w-3 bg-brand"></span> Thực tế</span>
            <span class="inline-flex items-center gap-1"><span class="h-0.5 w-3 border-t border-dashed border-slate-400"></span> Lý tưởng</span>
        </div>
    </div>
    <p v-else class="text-xs text-slate-400">Chưa đủ dữ liệu burndown.</p>
</template>
