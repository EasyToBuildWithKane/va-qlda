        ...
        <div class="grid min-h-0 flex-1 grid-cols-[3fr_7fr]">
            ...
            <div class="z-20 flex min-w-0 flex-col border-r border-slate-200 bg-slate-50/90">
                ...
                <div
                    ...
                    @scroll="onLeftScroll"
                >
                    <div class="relative w-full min-w-0" :style="{ height: `${totalHeight}px` }">
                        <div
                        ...
                        >
                            ...
                            >
                                <AppIcon :name="isGroupOpen(item.row.key) ? 'chevron-down' : 'chevron-right'" :size="14" class="shrink-0 text-slate-400" />
                                <div class="min-w-0 flex-1">
                                    <div class="flex flex-wrap items-center gap-1.5">
                                        <span class="text-xs font-semibold text-slate-800" :title="item.row.label">{{ item.row.label }}</span>
                                        <span
                                    ...
                                    <p v-if="item.row.meta" class="truncate text-[10px] text-slate-400">{{ item.row.meta }}</p>
                                </div>
                            ...
                            <button
                                ...
                                @click="toggleGroup(item.row.key)"
                            >
                                <AppIcon :name="isGroupOpen(item.row.key) ? 'chevron-down' : 'chevron-right'" :size="14" class="shrink-0 text-violet-400" />
                                <div class="min-w-0 flex-1">
                                    <span class="text-xs font-semibold text-violet-900" :title="item.row.label">{{ item.row.label }}</span>
                                    <p v-if="item.row.meta" class="truncate text-[10px] text-violet-600/80">{{ item.row.meta }}</p>
                                </div>
                ...
                <div :style="{ width: `${Math.max(timelineWidth, 640)}px`, minWidth: '100%' }">
                    ...
                    <div class="relative" :style="{ height: `${totalHeight}px` }">
                        ...
                        >
                            ...
                            <div v-if="item.row.type === 'sprint'" class="relative h-full bg-slate-50/70">
                                <div
                            ...
                            <!-- Milestone header bg -->
                            <div v-else-if="item.row.type === 'milestone-header'" class="h-full bg-violet-50/40"></div>

                            <!-- Milestone diamond -->
                            <div v-else-if="item.row.type === 'milestone'" class="relative h-full">
                                <button
                                ...
                                <div
                                    ...
                                    :style="{ left: `${milestonePercent(item.row.milestone)}%` }"
                                >
                                    <p class="font-semibold text-slate-800">{{ item.row.milestone.title }}</p>
                                    <p class="mt-1 text-slate-500">Hạn: {{ item.row.milestone.due_date || '—' }}</p>
                                    <p class="text-slate-500">Trạng thái: {{ item.row.milestone.status?.label }}</p>
                                    <p v-if="getAssignees(item.row.milestone)[0]" class="text-slate-500">
