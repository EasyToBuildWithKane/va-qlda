...
import DocumentPreviewPane from '@/Components/Project/Documents/DocumentPreviewPane.vue';
    ...
    <div class="flex h-full flex-col overflow-hidden">
        ...
        <div class="shrink-0 border-b border-slate-200 bg-white px-5 py-4 dark:border-slate-700 dark:bg-slate-900">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h2 class="font-display text-lg font-semibold text-slate-800 dark:text-slate-100">Tài liệu dự án</h2>
        ...
        <div class="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50/80 px-5 py-2.5 dark:border-slate-700 dark:bg-slate-900/50">
            <p class="text-xs text-slate-500 dark:text-slate-400">{{ activeCat?.description }}</p>
            <button
                v-if="canUpload"
                type="button"
            ...
            >
                <AppIcon :name="uploadingCategory === activeCategory ? 'refresh' : 'upload'" :size="14" :class="uploadingCategory === activeCategory ? 'animate-spin' : ''" />
        ...
        <div class="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[280px_1fr]">
            ...
            <div class="min-h-0 overflow-y-auto border-b border-slate-200 bg-white lg:border-b-0 lg:border-r dark:border-slate-700 dark:bg-slate-900">
                <div v-if="!categoryFiles.length" class="flex flex-col items-center justify-center px-4 py-16 text-center">
                    <AppIcon :name="activeCat?.icon || 'documents'" :size="32" class="text-slate-300" />
                    <p class="mt-2 text-sm text-slate-400">Chưa có tài liệu.</p>
                    <button
                        v-if="canUpload"
                        type="button"
                        class="mt-2 text-sm font-medium text-brand hover:underline"
                        @click="pickFiles(activeCategory)"
                    >Tải file đầu tiên</button>
                </div>
            ...
            <div class="flex min-h-0 flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
                    ...
                    <div class="min-h-0 flex-1 overflow-hidden p-4">
                        <div class="card flex h-full min-h-[240px] flex-col overflow-hidden dark:border-slate-700 dark:bg-slate-900">
                            <div class="flex shrink-0 items-center justify-between gap-2 border-b border-slate-100 px-4 py-2.5 dark:border-slate-700">
                                <h3 class="min-w-0 truncate font-medium text-slate-800 dark:text-slate-100">{{ selected.original_name }}</h3>
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <section class="flex flex-1 flex-col overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
                                    ...
                                    <div class="flex min-h-[88px] flex-1 flex-col px-3.5 py-2.5">
                                        ...
                                        <div v-if="editingNotes && canEdit" class="mt-2 flex justify-end gap-2 border-t border-slate-100 pt-2 dark:border-slate-800">
                                            <button
                                                type="button"
                                    ...
                                    >
                                        ...
                                        <input ref="replaceInput" type="file" class="hidden" :accept="acceptFor(activeCategory)" @change="onReplaceSelected" />
                                    </footer>
                                </section>
                            </div>
                            ...
                            <section class="flex min-h-[200px] flex-col overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900 lg:min-h-0">
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            <AppIcon :name="activityIcon(item.event)" :size="13" />
                                        </span>
                                        <div class="min-w-0 flex-1 pt-0.5">
                                            ...
                                            <p class="mt-1 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] text-slate-400">
                                                ...
                                                <span v-if="item.employee?.name" class="text-slate-300">·</span>
                                                <span v-if="item.employee?.name" class="font-medium text-slate-500">
                                                    {{ item.employee.name }}
                                                </span>
                                            </p>
                                        </div>
                                ...
                                >
                                    <span class="grid h-10 w-10 place-items-center rounded-full bg-slate-100 text-slate-400 dark:bg-slate-800">
                                        <AppIcon name="report-history" :size="18" />
                                    </span>
                                    <p class="mt-2 text-sm text-slate-500">Chưa có lịch sử</p>
                ...
                <div v-else class="flex flex-1 flex-col items-center justify-center p-8 text-center text-slate-400">
                    <AppIcon name="documents" :size="36" class="opacity-40" />
                    <p class="mt-3 text-sm">Chọn một tài liệu để xem preview và audit.</p>
                </div>
            </div>
        </div>
    </div>
