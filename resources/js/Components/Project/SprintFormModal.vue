...
<template>
    ...
    >
        <form class="space-y-5" @submit.prevent="submit">
            <div class="space-y-4">
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Thông tin sprint</p>
            ...
            <div class="space-y-4 rounded-xl border border-slate-200/80 bg-slate-50/50 p-4 dark:border-slate-700 dark:bg-slate-900/40">
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Thời gian & trạng thái</p>
                ...
                <div class="grid gap-3 sm:grid-cols-[1fr_auto_1fr] sm:items-end">
                    <div>
                        ...
                        <p v-if="form.errors.start_date" class="mt-1 text-xs text-danger">{{ form.errors.start_date }}</p>
                    </div>
                    <span class="hidden pb-2.5 text-slate-300 sm:block" aria-hidden="true">→</span>
                    <div>
                        <label class="label">Ngày kết thúc <span class="text-danger">*</span></label>
                        <input v-model="form.end_date" type="date" class="input" />
                        <p v-if="form.errors.end_date" class="mt-1 text-xs text-danger">{{ form.errors.end_date }}</p>
                    </div>
                </div>
                ...
                >
                    <AppIcon name="calendar" :size="14" class="shrink-0 text-slate-400" />
