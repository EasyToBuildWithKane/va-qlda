    ...
    <div class="grid grid-cols-1 gap-5 lg:grid-cols-4">
        <aside class="lg:col-span-1">
            <div class="space-y-4 lg:sticky lg:top-4">
                ...
                <div v-if="isCreate" class="card p-4">
                    <button
                        type="button"
                    ...
                    >
                        <h3 class="flex items-center gap-2 font-display text-sm font-semibold text-slate-800">
                            <AppIcon name="save" :size="16" class="text-brand" />
                            Bản nháp đã lưu
                            <span
                    ...
                    <div v-show="draftsOpen">
                        <p v-if="!draftStore.drafts.length" class="text-xs leading-relaxed text-slate-500">
                        ...
                        <ul v-else class="max-h-52 space-y-2 overflow-y-auto">
                            ...
                            >
                                <p class="truncate text-sm font-medium text-slate-800">{{ draftTitle(d) }}</p>
                                <p class="mt-0.5 text-[11px] text-slate-400">{{ formatDraftSavedAt(d.savedAt) }}</p>
                                <div class="mt-2 flex flex-wrap gap-1.5">
                                    <button type="button" class="btn-primary py-1 px-2.5 text-xs" @click="loadDraft(d)">
                                        Lấy bản nháp
                                    </button>
                                    <button type="button" class="btn-ghost py-1 px-2 text-xs text-rose-600" @click="deleteDraft(d)">
                                        Xóa
                                    </button>
                                </div>
                            </li>
        ...
        <form class="lg:col-span-3" @submit.prevent="submit('close')">
            <div
            ...
            >
                ...
                <button type="button" class="btn-primary shrink-0 text-sm" @click="loadDraft(draftStore.drafts[0])">
            ...
            <div class="card overflow-visible">
                <div class="flex flex-wrap border-b border-slate-200">
                    <button
                        ...
                        @click="activeTab = i"
                    >
                        <AppIcon :name="t.icon" :size="16" class="shrink-0 opacity-70" />
                        <span>{{ t.title }}</span>
                        <span
                ...
                <div class="overflow-visible p-6">
                    ...
                    <div v-show="activeTabMeta.key === 'basic'" class="space-y-4">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    tabindex="-1"
                                />
                                <p v-if="isCreate" class="mt-1 text-xs text-slate-400">Tự động sinh khi mở trang tạo dự án</p>
                    ...
                    <!-- Tab: Phạm vi -->
                    <div v-show="activeTabMeta.key === 'scope'" class="space-y-4">
                        <p class="text-xs text-slate-500">Chọn nơi dự án được áp dụng. <span class="text-danger">*</span></p>
                        ...
                        <p v-if="errFor('scope')" class="flex items-center gap-1 text-xs text-danger"><AppIcon name="close" :size="12" /> {{ errFor('scope') }}</p>

                        <div v-if="form.scope === 'regional'" class="rounded-card border border-amber-200 bg-amber-50/60 p-4">
                        ...
                        <div v-if="form.scope === 'departmental'" class="rounded-card border border-cyan-200 bg-cyan-50/60 p-4">
                        ...
                        <div v-if="!isCreate" class="border-t border-slate-100 pt-4">
                            <label class="label flex items-center gap-1.5">
                                Phòng ban phụ trách
                    ...
                    <!-- Tab: Thời gian -->
                    <div v-show="activeTabMeta.key === 'time'" class="space-y-4">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div>
                        ...
                        <p v-if="errFor('due_date')" class="flex items-center gap-1 text-xs text-danger"><AppIcon name="close" :size="12" /> {{ errFor('due_date') }}</p>
                    </div>

                    <!-- Tab: Nhân sự -->
                    <div v-show="activeTabMeta.key === 'people'" class="space-y-4">
