...
<template>
    <div class="space-y-3">
        ...
        <div v-for="p in modelValue" :key="p.id" class="rounded-card border border-slate-200 bg-white">
            ...
            <div class="border-t border-slate-100 px-3 py-2.5">
                <!-- Chosen tasks -->
                <div v-if="(p.tasks || []).length" class="mb-2 flex flex-wrap gap-1.5">
                    <span
                        v-for="t in p.tasks"
                        :key="t.id"
                ...
                >
                    <option value="" disabled>+ Thêm task của dự án này…</option>
                    <option v-for="t in availableTasks(p)" :key="t.id" :value="t.id">
                        {{ t.title }}{{ t.status ? ` — ${statusLabels[t.status] || t.status}` : '' }}
                    </option>
                </select>
                <p v-else-if="!(optionOf(p.id)?.tasks?.length)" class="text-xs text-slate-400">
                    Dự án này chưa có task nào.
                </p>
                <p v-else class="text-xs text-slate-400">Đã thêm tất cả task của dự án.</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2">
            <select
                ...
                @change="addProject"
            >
                <option value="" disabled>+ Thêm dự án…</option>
                <option v-for="o in available" :key="o.id" :value="o.id">{{ o.name }}</option>
            </select>
            <span v-else class="text-xs text-slate-400">Đã chọn tất cả dự án.</span>
        </div>

        <p v-if="!modelValue.length" class="text-xs text-slate-400">
