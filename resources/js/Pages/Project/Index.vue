...
import { COLUMNS, DEFAULT_VISIBLE } from '@/Components/Project/projectColumns';
    ...
    <AppLayout>
        ...
        <div class="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div v-for="c in cards" :key="c.label" class="card flex items-center gap-3 p-4">
                <span class="grid h-10 w-10 shrink-0 place-items-center rounded-btn" :class="c.bg">
                    <AppIcon :name="c.icon" :size="20" :class="c.tone" />
                </span>
                <div class="min-w-0">
        ...
        <div class="mb-3 flex flex-wrap items-center gap-2">
            ...
            <div v-if="view === 'list'" ref="exportBtnRef" class="relative">
                <button type="button" class="btn-ghost border border-slate-200" @click="showExportMenu = !showExportMenu">
                    <AppIcon name="download" :size="16" /> Xuất
                    <AppIcon name="chevron-down" :size="14" class="opacity-50" :class="showExportMenu && 'rotate-180'" />
                </button>
                <div
                    v-if="showExportMenu"
                    ref="exportMenuRef"
                ...
                >
                    <p class="px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide text-slate-400">Phạm vi</p>
                    ...
                    <button type="button" class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-slate-50" @click="runExport('xlsx')">
                        <AppIcon name="download" :size="15" class="text-brand" />
                        <span>
                            Excel (.xlsx)
                            <span class="block text-[10px] text-slate-400">Có KPI & định dạng VA</span>
                        </span>
                    </button>
                    <button type="button" class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-slate-50" @click="runExport('csv')">
                        <AppIcon name="download" :size="15" class="text-slate-500" />
                        <span>
                            CSV (.csv)
        ...
        <div v-else class="space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-2 text-sm">
                ...
                <div class="flex items-center gap-1">
                    <button type="button" class="rounded-btn border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50" @click="expandAllLanes">
                        Mở tất cả
                    </button>
                    <button type="button" class="rounded-btn border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50" @click="collapseAllLanes">
                        Thu gọn tất cả
                    </button>
                </div>
            </div>
            ...
            >
                ...
                >
                    ...
                    />
                    <span class="h-2.5 w-2.5 shrink-0 rounded-full" :class="dot[col.color] || dot.slate"></span>
                    ...
                    <div v-if="col.projects.length" class="ml-2 flex items-center gap-1">
                        <template v-for="(p, i) in col.projects.slice(0, 5)" :key="p.id">
                            <Avatar v-if="p.manager" :name="p.manager.name" :src="p.manager.avatar_path" :size="20" :title="p.manager.name" :style="{ marginLeft: i === 0 ? '0' : '-4px' }" class="rounded-full ring-1 ring-white" />
                        </template>
                        <span v-if="col.projects.length > 5" class="ml-1 text-xs text-slate-400">+{{ col.projects.length - 5 }} khác</span>
                    </div>
                </button>

                <!-- Horizontal card row -->
                <div v-show="isLaneOpen(col.key)" class="kanban-row flex gap-3 overflow-x-auto pb-1">
                    <ProjectCard
