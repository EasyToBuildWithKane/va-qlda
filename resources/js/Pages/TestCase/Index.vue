...
const testSuiteOptions = computed(() => props.options.testSuites ?? []);
...
<template>
  ...
  <AppLayout>
    <template #header>
      <PageHeader
        title="QA / Test case"
        subtitle="Danh sách trường hợp kiểm thử — lọc, thực thi và theo dõi kết quả"
        icon="check-circle"
        icon-color="brand"
    ...
    <div class="card overflow-visible">
      ...
      <div
        v-if="hasFilterRow"
        class="grid grid-cols-1 gap-3 border-b border-slate-100 px-5 py-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6"
      >
        <DatagridFilterField
        ...
        >
          <SearchSelect
            :model-value="filterForm.project_id || null"
            :options="projectOptions"
            placeholder="Dự án"
            search-placeholder="Tìm tên hoặc mã dự án…"
            :search-keys="['name', 'code']"
            @update:model-value="(v) => { filterForm.project_id = v ?? ''; applyFilters({ page: 1 }); }"
          />
        </DatagridFilterField>
      ...
      <div
        ...
        class="flex items-center gap-2 border-b border-slate-100 px-5 py-2.5"
      >
          ...
          <tbody>
            <template v-if="testCaseList.length">
              ...
              >
                ...
                >
                  {{ displayOrEmpty(tc.project?.code || tc.project?.name, 'Chưa gán dự án') }}
                </td>
                <td
                  v-if="isColVisible('suite')"
                  class="px-4 py-3 text-xs text-slate-500"
                ...
                <div class="flex flex-col items-center gap-3 text-center">
                  ...
                  <div>
                    ...
                    <p class="mt-1 text-xs text-slate-400">
                      {{ appliedFilterCount > 0 ? 'Thử thay đổi bộ lọc.' : 'Bấm «Thêm test case» để bắt đầu.' }}
                    </p>
                  </div>
                  <button
