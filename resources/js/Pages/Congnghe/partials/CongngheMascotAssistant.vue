...
const {
    ...
    hideAssistant,
    showAssistant,
} = useCongngheAssistantDock(asideRef, { onTap: togglePanel });
...
<template>
  ...
  <aside
    v-else
    ref="asideRef"
    class="cn-assistant fixed z-[45] flex w-[min(calc(100vw-1.25rem),19rem)] flex-col items-end sm:w-[20.5rem]"
  ...
  >
    ...
    <div class="cn-assistant-dock relative flex w-full justify-end">
      <button
        type="button"
        class="cn-assistant-mascot group relative touch-none select-none border-0 bg-transparent p-0"
        :class="isDragging ? 'cursor-grabbing' : 'cursor-grab'"
        :aria-expanded="panelOpen"
        aria-label="Trợ lý VAS — bấm để mở hộp thoại, giữ và kéo để di chuyển"
    ...
    >
      Giữ linh vật để kéo · Bấm linh vật để {{ panelOpen ? 'thu gọn' : 'mở' }}
    </p>
  </aside>
</template>
