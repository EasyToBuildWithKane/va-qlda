...
const reportSummary = computed(() => {
    if (props.summary?.contracts != null) {
        return props.summary;
    }
    const base = props.report?.byVendor ?? [];
    return {
        contracts: base.reduce((s, r) => s + (r.count || 0), 0),
        vendors: base.length,
        annual_cost: base.reduce((s, r) => s + (Number(r.annual_cost) || 0), 0),
        lifecycle_cost: base.reduce((s, r) => s + (Number(r.lifecycle_cost) || 0), 0),
    };
});
  ...
  <AppLayout>
    <template #header>
      <PageHeader
        ...
        icon="team-eval"
        icon-color="brand"
        :badge="reportSummary.contracts ?? null"
      />
    </template>

    <ReportSummaryBar :summary="reportSummary" />
